...
void add_job_item(GtkButton* button, gpointer data)
{
    struct clients_data {
        char punkts[18];
    ...
    if (checked_string(client_data.total_price, s->window) == -1)
        return;
    client_data.total_percent = gtk_entry_get_text((GtkEntry*)s->entry_total_percent);
    if (checked_string(client_data.total_percent, s->window) == -1)
        return;
...
void add_job(GtkButton* button, gpointer data)
{
    struct clients_data {
        const char* name;
        const char* address;
...
void add_tables(GtkToolButton* toolbutton, gpointer user_data)
{
    mysql_query(&mysql,
...
void get_job_items(GtkToolButton* toolbutton, gpointer data)
{
    ...
    char* query = malloc(1024);
    if (!query) {
        GtkWidget *message = gtk_message_dialog_new((GtkWindow*)gji->window,
           GTK_DIALOG_MODAL,
           GTK_MESSAGE_ERROR,
           GTK_BUTTONS_CLOSE,
           "Ну удалось выделить память, попробуйте снова.");
...
void activate_job(GtkTreeView* tree_view, GtkTreePath* path, GtkTreeViewColumn* column,
    gpointer data)
{
		struct get_job_info *gji = ( struct get_job_info* ) data;
    ...
    gpointer data)
{
    ...
    if (gtk_tree_selection_get_selected(select, &model, &iter)) {
        ...
        g_signal_connect((GtkWidget*)gji->client_id, "row-activated", G_CALLBACK(activate_job), gji);
        gtk_container_add((GtkContainer*)window, main_box);
        gtk_widget_show_all((GtkWidget*)window);
    }
}

void get_clients(GtkToolButton* toolbutton, gpointer data)
{
...
void delete_event(GtkApplication* app, GtkWindow* window, gpointer user_data)
{
    mysql_close(&mysql);
}
struct search_item {
...
void search_item(GtkSearchEntry* entry, gpointer data)
{
    ...
    if (!length_of_search) {
        ...
        while (row = mysql_fetch_row(res)) {
            lengths = mysql_fetch_lengths(res);
            int total_length = mysql_num_fields(res);
            if (g_strrstr(row[1], search_str) || g_strrstr(row[2], search_str) || g_strrstr(row[3], search_str)) {
                gtk_tree_store_append(si->store_id_clients, &iter_id, NULL);
                gtk_tree_store_set(si->store_id_clients, &iter_id,
                    ID_CLIENT, row[0],
                    NAME_CLIENT, row[1],
...
void show_many(GtkButton* button, gpointer data)
{
    struct date_c {
        guint year;
        guint month;
...
void many(GtkToolButton* toolbutton, gpointer data)
{
...
activate(GtkApplication* app, gpointer user_data)
{
    ...
    g_signal_connect((GtkWidget*)si->clients_id, "row-activated", G_CALLBACK(activate_client), main_window);
